...
listController.Service = (req, res, next) => {
    ...
    db.all(`SELECT access_key, secret_key, region, cluster_name FROM Credentials WHERE user_id = ?`, [userId], async (err, rows) => {
        if (err) {
            return res.status(500).json({error: 'Error occurred during query the database'});
        } else {
            if (rows.length === 0) {
                return res.status(404).json({error: 'No credentials found for the use, configure Credentials first'});
            }
            ...
            try {
                ...
                if (listServiceData.serviceArns.length === 0) {
                    return res.status(404).json({error: 'No services found in the specific cluster'});
                }
                ...
                const describeServiceData = await client.send(describeServicesCommand);
                res.json(describeServiceData.services.map(service => service.serviceName));
              } catch (err) {
